...
public static class TvGradeSweep
{
    ...
    // Espera tras inyectar el INIT: los bundles de decos y peces tardan en cargar.
    private const double EsperaCargaSeg  = 12.0;
    ...
    private const string EstadoPorDefecto = @"{
        ""activeFish"": [
            ...
            {""speciesId"": ""fish_clownfish"", ""nickname"": ""b""}
        ],
    ...
    // ── Máquina de estados ───────────────────────────────────────────────────
    // paso 0        = esperando a que arranque el play mode
    // paso 1        = INIT inyectado, esperando a que carguen los bundles
    ...
    private static void Tick()
    {
        ...
        if (paso < 0) return;
        if (!Application.isPlaying) return;              // aún entrando en play
        ...
        if (paso == 0)
        {
            ...
            Programar(paso + 1, EsperaCargaSeg);
            Debug.Log($"[GRADE] INIT inyectado. Esperando {EsperaCargaSeg:F0} s a que carguen los bundles…");
            return;
        }
    ...
    private static bool Capturar(string nombre, int indice)
    {
        ...
        var rtPrevia   = cam.targetTexture;
